...
final class AppModel {
    ...
    func reconcileLiveIdentities() async {
        for provider in providers {
            ...
            await Task.detached { await KeychainGate.shared.run { provider.consolidateLive(credentials: credentials) } }
                .value
    ...
    private func watchLogin(for provider: AccountProvider, before: String?) async {
        ...
        while Date() < deadline {
            ...
            if let identity, identity.handle != before {
                ...
                await Task.detached {
                    await KeychainGate.shared.run { provider.consolidateLive(credentials: credentials) }
                }.value
                await adopt(identity: identity, for: provider)
                await refreshUsage()
                finishLogin(for: provider.id, error: nil)
